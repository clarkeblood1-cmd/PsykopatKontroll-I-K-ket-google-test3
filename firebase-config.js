(function () {
  ...
  function collectState() {
    return {
      ...
      appVersion: 'cloud-sync-fast-ui-v3'
    };
  }

  function applyRemoteState(data) {
    const remoteUpdatedAtMs = Math.max(0, Number(data?.updatedAtMs || 0));
    if (remoteUpdatedAtMs && remoteUpdatedAtMs <= lastAppliedRemoteMs) {
      return;
    }
    ...
    try {
      ...
      if (remoteUpdatedAtMs) lastAppliedRemoteMs = remoteUpdatedAtMs;
    } finally {
