...
function isRecipeMeatPortionIngredient(ingredient) {
  const ing = normalizeRecipeIngredient(ingredient);
  if (!ing) return false;

  const category = normalizeCategoryName(ing.category || '');
  if (category === 'KÖTTFÄRS' || category === 'KOTTFARS') return true;

  return isMincedMeatRecipeIngredient(ing) || shouldUseGlobalPortionWeight(ing, { category: 'matlagning', type: 'matlagning' });
}
...
function applySelectedPortionToIngredient(value, recipe = null) {
  ...
  if (!isRecipeMeatPortionIngredient(ingredient)) {
    return { ...ingredient };
  }

  const meatGrams = Math.max(1, Number(getSelectedRecipePortions() || 1) * Number(portionGrams || 0));
  return {
    ...ingredient,
    quantity: 1,
    unit: 'g',
    size: meatGrams,
    packMeasureUnit: '',
    measureText: formatSmartMeasureDisplay(meatGrams, 'g'),
    weightText: formatSmartMeasureDisplay(meatGrams, 'g')
  };
}
...
function normalizeMeasureItemData(item, unitHint = null) {
  ...
  const originalUnit = String(item?.unit || '').toLowerCase();
  const cameFromPackUnit = isPackUnit(originalUnit);
  const hint = unitHint || (cameFromPackUnit ? getPackMeasureUnit(item, 'g') : item.unit) || 'g';
  ...
  const openedAmount = getUnitFamily(hint) === 'weight'
    ...
    : 0;
  const shouldKeepPackMode = getUnitFamily(hint) === 'weight'
...
function updateIngredientEditMeasureSummary() {
  ...
  const packUnitEl = document.getElementById('ingredientEditPackMeasureUnit');

  if (!amountEl || !unitEl || !inputEl || !summaryEl) return;

  const amount = Math.max(1, Number(amountEl.value || 1));
  const unit = String(unitEl.value || 'st').toLowerCase();
  const measureUnit = isPackUnit(unit)
    ? String(packUnitEl?.value || 'g').toLowerCase()
    : unit;
  const parsed = parseSmartMeasureInput(inputEl.value, measureUnit);
...
function syncIngredientEditMeasureModeVisibility() {
  ...
  const unit = String(unitEl.value || 'st').toLowerCase();
  const smartMode = supportsMeasureInput(unit);
  const measureUnit = isPackUnit(unit)
    ? String(packUnitEl?.value || 'g').toLowerCase()
    : unit;
  ...
  if (packUnitEl) packUnitEl.style.display = isPackUnit(unit) ? '' : 'none';

  if (inputEl) inputEl.placeholder = getSmartMeasurePlaceholder(unit);

  if (smartMode && inputEl?.value) {
    const parsed = parseSmartMeasureInput(inputEl.value, measureUnit);
    if (parsed) inputEl.value = formatSmartMeasureDisplay(parsed, measureUnit);
  }
...
function createCard(item, source = 'items') {
  ...
  div.innerHTML = `
    ...
    <div class="info">
      ...
      <div class="meta">
        <div class="quantity-wrap">
          ${item.type === 'home' && (isPackTrackedItem(item) || isPackUnit(item.unit))
            ? `<span>${formatItemAmount(item)}</span>`
...
function editDraftIngredient(index) {
  ...
  const ingredientEditPackMeasureUnit = document.getElementById('ingredientEditPackMeasureUnit');
  ...
  if (ingredientEditPackMeasureUnit) ingredientEditPackMeasureUnit.value = getPackMeasureUnit(parsed, 'g');
...
function normalizeRecipeIngredient(value) {
  ...
  const preservedCategory = normalizeCategoryName(value.category || '');
...
function buildRecipeIngredient(name, quantity, unit, size = null, category = '', recipe = null, smartMode = 'linked', packMeasureUnit = '') {
  return syncRecipeIngredientFromQuickItem({ name, quantity, unit, size, category, smartMode, packMeasureUnit }, recipe);
}
...
function saveIngredientEdit() {
  ...
  const unit = String(document.getElementById('ingredientEditUnit')?.value || 'st').toLowerCase();
  const packMeasureUnit = isPackUnit(unit)
  ...
  if (supportsMeasureInput(unit)) {
    const parsedMeasure = parseSmartMeasureInput(document.getElementById('ingredientEditMeasureText')?.value || size, measureUnit);
    if (!parsedMeasure) {
      alert(getMeasureInputError(unit));
      return;
    }
    const measureWarning = getMeasureValidationMessage(parsedMeasure, quantity, measureUnit);
    if (measureWarning) {
      alert(measureWarning);
      return;
    }
    ...
    if (ingredientEditMeasureInput) ingredientEditMeasureInput.value = formatSmartMeasureDisplay(parsedMeasure, measureUnit);
  }
...
document.addEventListener('change', (e) => {
  ...
  if (e.target.id === 'ingredientEditUnit' || e.target.id === 'ingredientEditPackMeasureUnit') syncIngredientEditMeasureModeVisibility();
  ...
  if (e.target.id === 'ingredientEditMeasureText') { const ingredientUnit = String(document.getElementById('ingredientEditUnit')?.value || 'g').toLowerCase(); const ingredientMeasureUnit = isPackUnit(ingredientUnit) ? String(document.getElementById('ingredientEditPackMeasureUnit')?.value || 'g').toLowerCase() : ingredientUnit; formatMeasureInputField(e.target, ingredientMeasureUnit); }
});
...
document.addEventListener('blur', (e) => {
  ...
  if (e.target.id === 'ingredientEditMeasureText') {
    const ingredientUnit = String(document.getElementById('ingredientEditUnit')?.value || 'g').toLowerCase();
    const ingredientMeasureUnit = isPackUnit(ingredientUnit) ? String(document.getElementById('ingredientEditPackMeasureUnit')?.value || 'g').toLowerCase() : ingredientUnit;
    formatMeasureInputField(e.target, ingredientMeasureUnit);
    updateIngredientEditMeasureSummary();
  }
}, true);
...
(function () {
  ...
  function getQuickPackTemplateForIngredient(ingredient) {
    const ing = typeof normalizeRecipeIngredient === 'function'
      ? normalizeRecipeIngredient(ingredient)
      : ingredient;
    if (!ing || typeof findQuickItemByName !== 'function') return null;
    const quick = findQuickItemByName(ing.name || '');
    if (!quick) return null;
    const unit = String(quick.unit || ing.unit || 'st').toLowerCase();
    const size = Math.max(0, Number(quick.size || 0));
    if (!isWeightUnit(unit) || size <= 0) return null;
    return { ...quick, unit, size };
  }

  function takeOneFullPackFromHomeByTemplate(template) {
    if (!template || !Array.isArray(items)) return false;

    const matches = items
      .map((item, index) => ({ item, index }))
      .filter(({ item }) => item && item.type === 'home' && sameName(item.name, template.name) && isWeightUnit(item.unit));

    if (!matches.length) return false;

    const packSize = Math.max(1, Number(template.size || 0));

    matches.sort((a, b) => {
      const aOpened = Math.max(0, Number(a.item.openedAmount || 0));
      const bOpened = Math.max(0, Number(b.item.openedAmount || 0));
      const aQty = Math.max(0, Number(a.item.quantity || 0));
      const bQty = Math.max(0, Number(b.item.quantity || 0));
    ...
    for (const { item, index } of matches) {
      ...
      const currentPackSize = Math.max(1, Number(current.size || packSize));
      const currentQty = Math.max(0, Number(current.quantity || 0));
      const currentOpened = Math.max(0, Number(current.openedAmount || 0));
      let totalAmount = currentQty * currentPackSize + currentOpened;
      if (totalAmount <= 0) continue;

      totalAmount = Math.max(0, totalAmount - packSize);
      consumed = true;
      ...
      const unopened = Math.floor(totalAmount / currentPackSize);
      const opened = totalAmount % currentPackSize;

      current.quantity = Math.max(0, unopened);
      current.openedAmount = Math.max(0, opened);
      current.packMode = (current.quantity > 1 || current.openedAmount > 0 || current.packMode === 'bags') ? 'bags' : '';
      current.measureText = typeof formatSmartMeasureDisplay === 'function' ? formatSmartMeasureDisplay(currentPackSize, current.unit) : (current.measureText || '');
      current.weightText = typeof formatSmartMeasureDisplay === 'function' ? formatSmartMeasureDisplay(currentPackSize, current.unit) : (current.weightText || '');

      if (current.quantity <= 0 && current.openedAmount <= 0) {
        items.splice(index, 1);
      }
      break;
    }
  ...
  function addExactlyOnePackToBuyByTemplate(template) {
    if (!template) return;
    if (typeof addBuyItemFromTemplate === 'function') {
      addBuyItemFromTemplate(template, 1);
      return;
    }
    items.push({
      name: template.name || '',
      ...
      size: Math.max(1, Number(template.size || 0)),
      measureText: typeof formatSmartMeasureDisplay === 'function' ? formatSmartMeasureDisplay(Math.max(1, Number(template.size || 0)), template.unit || 'g') : '',
      weightText: typeof formatSmartMeasureDisplay === 'function' ? formatSmartMeasureDisplay(Math.max(1, Number(template.size || 0)), template.unit || 'g') : '',
      category: template.category || (typeof getRoomFallbackCategory === 'function' ? getRoomFallbackCategory(template.room || activeRoom) : 'ÖVRIGT'),
      place: template.place || 'kyl',
      img: template.img ? String(template.img) : '',
      type: 'buy',
      openedAmount: 0,
      packMode: ''
    });
  ...
  const originalUseRecipeIngredientsPackRule = window.useRecipeIngredients || useRecipeIngredients;
  window.useRecipeIngredients = function useRecipeIngredientsQuickTemplatePackRule() {
    ...
    (Array.isArray(combinedEntries) ? combinedEntries : []).forEach(entry => {
      ...
      const template = getQuickPackTemplateForIngredient(ingredient);
      if (template) packEntries.push({ entry, ingredient, template });
      else normalEntries.push(entry);
    });

    if (typeof consumeIngredientEntries === 'function' && normalEntries.length) {
      consumeIngredientEntries(normalEntries);
    }

    packEntries.forEach(({ template }) => {
      const didConsume = takeOneFullPackFromHomeByTemplate(template);
      if (didConsume) addExactlyOnePackToBuyByTemplate(template);
    });

    if (typeof mergeItems === 'function') items = mergeItems(items);
  ...
  window.__autoBuyZip = 'v62-recipe-follows-quick-pack-template';
})();
...
(function () {
  ...
  const originalUseRecipeIngredientsV63 = window.useRecipeIngredients || useRecipeIngredients;

  function isQuickTemplatePackItem(template) {
    if (!template) return false;
    const unit = String(template.unit || 'st').toLowerCase();
    return typeof supportsSize === 'function' && supportsSize(unit) && Number(template.size || 0) > 0;
  }

  function findMatchingHomePackIndex(template) {
    if (!template || !Array.isArray(items)) return -1;
    const wantedName = normalizeText(template.name || '');
    const wantedUnit = String(template.unit || 'st').toLowerCase();
    const wantedSize = Number(template.size || 0);

    return items.findIndex(entry => {
      if (!entry || entry.type !== 'home') return false;
      if (normalizeText(entry.name || '') !== wantedName) return false;
      if (String(entry.unit || 'st').toLowerCase() !== wantedUnit) return false;
      if (wantedSize > 0 && Number(entry.size || 0) !== wantedSize) return false;
      return Number(entry.quantity || 0) > 0 || Number(entry.openedAmount || 0) > 0;
    });
  }

  function removeExactlyOnePackFromHome(template) {
    const index = findMatchingHomePackIndex(template);
    if (index < 0) return false;

    const entry = items[index];
    if (!entry) return false;

    const unopened = Math.max(0, Number(entry.quantity || 0));
    const opened = Math.max(0, Number(entry.openedAmount || 0));

    if (unopened > 0) {
      entry.quantity = unopened - 1;
    } else if (opened > 0) {
      entry.openedAmount = 0;
    }

    entry.quantity = Math.max(0, Number(entry.quantity || 0));
    entry.openedAmount = Math.max(0, Number(entry.openedAmount || 0));
    entry.packMode = isWeightUnit(entry.unit || 'st') && (entry.quantity > 1 || entry.openedAmount > 0 || entry.packMode === 'bags') ? 'bags' : '';

    if (entry.quantity <= 0 && entry.openedAmount <= 0) {
      items.splice(index, 1);
    }
  ...
  function addExactlyOnePackToBuy(template) {
    if (!template) return;
    if (typeof addBuyItemFromTemplate === 'function') {
      addBuyItemFromTemplate(template, 1);
      return;
  ...
  window.useRecipeIngredients = function useRecipeIngredientsQuickTemplatePackLock() {
    const recipe = typeof getSelectedRecipe === 'function' ? getSelectedRecipe() : null;
    if (!recipe) return;
    ...
    combinedEntries.forEach(entry => {
      const ingredient = normalizeRecipeIngredient(entry?.ingredient || entry);
      const quickTemplate = ingredient && typeof findQuickItemByName === 'function' ? findQuickItemByName(ingredient.name || '') : null;
      if (ingredient && isQuickTemplatePackItem(quickTemplate)) {
        packEntries.push({ ingredient, quickTemplate });
      } else {
    ...
    packEntries.forEach(({ quickTemplate }) => {
      const removed = removeExactlyOnePackFromHome(quickTemplate);
      if (removed) addExactlyOnePackToBuy(quickTemplate);
    });
  ...
  window.__autoBuyZip = 'v63-one-pack-per-cook';
})();
...
(function () {
  ...
  function normalizeSafeText(value) {
    try {
      return typeof normalizeText === 'function' ? normalizeText(value || '') : String(value || '').trim().toLowerCase();
    } catch (e) {
      return String(value || '').trim().toLowerCase();
    }
  }

  function isTemplatePack(template) {
    if (!template) return false;
    const unit = String(template.unit || 'st').toLowerCase();
    return typeof supportsSize === 'function' && supportsSize(unit) && Number(template.size || 0) > 0;
  }

  function getQuickTemplateByIngredient(ingredient) {
    if (!ingredient || typeof findQuickItemByName !== 'function') return null;
    return findQuickItemByName(ingredient.name || '') || null;
  }

  function findPackIndexByTemplate(type, template) {
    if (!template || !Array.isArray(items)) return -1;
    const wantedName = normalizeSafeText(template.name || '');
    const wantedUnit = String(template.unit || 'st').toLowerCase();
    const wantedSize = Math.max(0, Number(template.size || 0));
    return items.findIndex(entry => {
      if (!entry) return false;
      if ((entry.type === 'buy' ? 'buy' : 'home') !== type) return false;
      if (normalizeSafeText(entry.name || '') != wantedName) return false;
      if (String(entry.unit || 'st').toLowerCase() !== wantedUnit) return false;
      if (wantedSize > 0 && Number(entry.size || 0) !== wantedSize) return false;
      return Number(entry.quantity || 0) > 0 || Number(entry.openedAmount || 0) > 0;
    });
  ...
  function removeOnePackFromList(type, template) {
    const index = findPackIndexByTemplate(type, template);
    if (index < 0) return false;
    const entry = items[index];
    if (!entry) return false;
    ...
    const unopened = Math.max(0, Number(entry.quantity || 0));
    const opened = Math.max(0, Number(entry.openedAmount || 0));
    if (unopened > 0) entry.quantity = unopened - 1;
    else if (opened > 0) entry.openedAmount = 0;
  ...
  function addOnePackToBuy(template) {
    if (!template) return;
    if (typeof addBuyItemFromTemplate === 'function') {
      addBuyItemFromTemplate(template, 1);
      return;
  ...
  function addOnePackToHome(template, targetPlace) {
    if (!template) return;
    const place = targetPlace || template.place || 'kyl';
    if (typeof addHomeItemFromTemplate === 'function') {
      addHomeItemFromTemplate({ ...template, type: 'home', place }, 1, place);
      return;
    }
    items.push({
      name: template.name || '',
  ...
  function persistAndRefresh() {
    ...
    try { if (typeof window.saveToCloudNow === 'function') window.saveToCloudNow(); } catch (e) {}
    try { if (typeof checkRecipe === 'function') checkRecipe(); } catch (e) {}
    try { if (typeof render === 'function') render(); } catch (e) {}
  }

  const originalTransferSingleItemFinal = window.transferSingleItem || transferSingleItem;
  window.transferSingleItem = function transferSingleItemFinalPackSave(index, targetType, targetPlace = null) {
    const sourceItem = items[index];
    if (!sourceItem) return;

    const sourceType = sourceItem.type === 'buy' ? 'buy' : 'home';
    const normalizedTargetType = targetType === 'buy' ? 'buy' : 'home';
    const template = getQuickTemplateByIngredient(sourceItem);

    if (template && isTemplatePack(template)) {
      if (sourceType === 'home' && normalizedTargetType === 'buy') {
        if (removeOnePackFromList('home', template)) {
          addOnePackToBuy(template);
          persistAndRefresh();
        }
        return;
      }
      if (sourceType === 'buy' && normalizedTargetType === 'home') {
        if (removeOnePackFromList('buy', template)) {
          addOnePackToHome(template, targetPlace || sourceItem.place || template.place || 'kyl');
          persistAndRefresh();
        }
        return;
      }
    ...
    const result = originalTransferSingleItemFinal.apply(this, arguments);
    persistAndRefresh();
    return result;
  };
  transferSingleItem = window.transferSingleItem;

  const originalUseRecipeIngredientsFinal = window.useRecipeIngredients || useRecipeIngredients;
  window.useRecipeIngredients = function useRecipeIngredientsFinalPackSave() {
    const recipe = typeof getSelectedRecipe === 'function' ? getSelectedRecipe() : null;
    if (!recipe) return;
    ...
    (Array.isArray(combinedEntries) ? combinedEntries : []).forEach(entry => {
      const ingredient = typeof normalizeRecipeIngredient === 'function'
        ? normalizeRecipeIngredient(entry?.ingredient || entry)
        : (entry?.ingredient || entry);
      const template = getQuickTemplateByIngredient(ingredient);
      if (template && isTemplatePack(template)) packTemplates.push(template);
      else normalEntries.push(entry);
    });

    if (normalEntries.length && typeof consumeIngredientEntries === 'function') {
      consumeIngredientEntries(normalEntries);
    }
  ...
  window.__autoBuyZip = 'v64-save-fix';
})();
...
(function () {
  ...
  function normalizeSafeTextV65(value) {
    try {
      return typeof normalizeText === 'function'
        ? normalizeText(value || '')
  ...
  function isQuickTemplatePackV65(template) {
    if (!template) return false;
    const unit = String(template.unit || 'st').toLowerCase();
    const size = Math.max(0, Number(template.size || 0));
    return size > 0 && ((typeof isPackUnit === 'function' && isPackUnit(unit)) || (typeof supportsSize === 'function' && supportsSize(unit)));
  }

  function getQuickTemplateV65(value) {
    const name = typeof value === 'string' ? value : (value && value.name) || '';
    if (!name || typeof findQuickItemByName !== 'function') return null;
    return findQuickItemByName(name) || null;
  }

  function findPackEntryIndexV65(type, template) {
    if (!template || !Array.isArray(items)) return -1;
    const wantedName = normalizeSafeTextV65(template.name || '');
    const wantedUnit = String(template.unit || 'st').toLowerCase();
    const wantedSize = Math.max(0, Number(template.size || 0));
    return items.findIndex((entry) => {
      if (!entry) return false;
      if ((entry.type === 'buy' ? 'buy' : 'home') !== type) return false;
      if (normalizeSafeTextV65(entry.name || '') !== wantedName) return false;
      if (String(entry.unit || 'st').toLowerCase() !== wantedUnit) return false;
      if (wantedSize > 0 && Math.max(0, Number(entry.size || 0)) !== wantedSize) return false;
      return Number(entry.quantity || 0) > 0 || Number(entry.openedAmount || 0) > 0;
    });
  }

  function removeExactlyOnePackV65(type, template) {
    const index = findPackEntryIndexV65(type, template);
    if (index < 0) return false;
    const entry = items[index];
    const quantity = Math.max(0, Number(entry.quantity || 0));
    const openedAmount = Math.max(0, Number(entry.openedAmount || 0));

    if (quantity > 0) {
      entry.quantity = quantity - 1;
    } else if (openedAmount > 0) {
      entry.openedAmount = 0;
    } else {
      return false;
    }
  ...
  function addExactlyOnePackToBuyV65(template) {
    if (!template) return;
    const fixedTemplate = {
      ...template,
      quantity: 1,
      size: Math.max(1, Number(template.size || 0)),
      measureText: template.measureText || (typeof getMeasureTextFromSize === 'function'
        ? getMeasureTextFromSize(Math.max(1, Number(template.size || 0)), (typeof isPackUnit === 'function' && isPackUnit(template.unit || 'st')) ? (template.packMeasureUnit || 'g') : (template.unit || 'g'))
        : ''),
      weightText: template.weightText || template.measureText || ''
    };
    if (typeof addBuyItemFromTemplate === 'function') {
      addBuyItemFromTemplate(fixedTemplate, 1);
      return;
    }
    items.push({ ...fixedTemplate, type: 'buy', openedAmount: 0, packMode: '' });
  }

  function addExactlyOnePackToHomeV65(template, targetPlace) {
    if (!template) return;
    const fixedTemplate = {
      ...template,
      quantity: 1,
      size: Math.max(1, Number(template.size || 0)),
      place: targetPlace || template.place || 'kyl'
    };
    if (typeof addHomeItemFromTemplate === 'function') {
      addHomeItemFromTemplate(fixedTemplate, 1, fixedTemplate.place);
      return;
    }
    items.push({ ...fixedTemplate, type: 'home', openedAmount: 0, packMode: '' });
  }

  function persistAndRefreshV65() {
    try {
      if (typeof mergeItems === 'function') items = mergeItems(items);
    } catch (error) {}
    try {
      ...
      console.error('Persist error v65:', error);
    }
    try { if (typeof window.saveToCloudNow === 'function') window.saveToCloudNow(); } catch (error) {}
    try { if (typeof checkRecipe === 'function') checkRecipe(); } catch (error) {}
    try { if (typeof render === 'function') render(); } catch (error) {}
  }

  const previousTransferSingleItemV65 = window.transferSingleItem || transferSingleItem;
  window.transferSingleItem = function transferSingleItemV65(index, targetType, targetPlace = null) {
    const sourceItem = items[index];
    if (!sourceItem) return;

    const sourceType = sourceItem.type === 'buy' ? 'buy' : 'home';
    const normalizedTargetType = targetType === 'buy' ? 'buy' : 'home';
    const template = getQuickTemplateV65(sourceItem);

    if (template && isQuickTemplatePackV65(template)) {
      if (sourceType === 'home' && normalizedTargetType === 'buy') {
        if (removeExactlyOnePackV65('home', template)) {
          addExactlyOnePackToBuyV65(template);
          persistAndRefreshV65();
        }
        return;
      }
      if (sourceType === 'buy' && normalizedTargetType === 'home') {
        if (removeExactlyOnePackV65('buy', template)) {
          addExactlyOnePackToHomeV65(template, targetPlace || sourceItem.place || template.place || 'kyl');
          persistAndRefreshV65();
        }
        return;
      }
    ...
    const result = previousTransferSingleItemV65.apply(this, arguments);
    persistAndRefreshV65();
    return result;
  };
  transferSingleItem = window.transferSingleItem;

  const previousUseRecipeIngredientsV65 = window.useRecipeIngredients || useRecipeIngredients;
  window.useRecipeIngredients = function useRecipeIngredientsV65() {
    const recipe = typeof getSelectedRecipe === 'function' ? getSelectedRecipe() : null;
    if (!recipe) return;
    ...
    (Array.isArray(combinedEntries) ? combinedEntries : []).forEach((entry) => {
      const ingredient = typeof normalizeRecipeIngredient === 'function'
        ? normalizeRecipeIngredient(entry?.ingredient || entry)
        : (entry?.ingredient || entry);
      const template = getQuickTemplateV65(ingredient);
      if (template && isQuickTemplatePackV65(template)) packTemplates.push(template);
      else normalEntries.push(entry);
    });
  ...
  window.__autoBuyZip = 'v65-quick-pack-sync';
})();
...
(function () {
  ...
  function clonePackTemplateToHomeV66(sourceItem, quantity, targetPlace) {
    if (!sourceItem) return null;
    const packUnit = (typeof getPackMeasureUnit === 'function') ? getPackMeasureUnit(sourceItem, 'g') : 'g';
    const rawSize = (typeof parseSmartMeasureInput === 'function')
      ? (parseSmartMeasureInput(sourceItem.measureText || sourceItem.weightText || sourceItem.size, packUnit) || sourceItem.size)
      : sourceItem.size;
    const packSize = Math.max(1, Number(rawSize || 0));
    const qty = Math.max(1, Number(quantity || 1));
    const roomKey = sourceItem.room || (typeof activeRoom !== 'undefined' ? activeRoom : 'koket');
    const placeKey = (typeof ensurePlaceExists === 'function')
      ? ensurePlaceExists(targetPlace || sourceItem.place || 'kyl', roomKey)
      : (targetPlace || sourceItem.place || 'kyl');
    const categoryValue = (typeof ensureCategoryExists === 'function')
      ? ensureCategoryExists(sourceItem.category || (typeof getRoomFallbackCategory === 'function' ? getRoomFallbackCategory(roomKey) : 'ÖVRIGT'), roomKey)
      : (sourceItem.category || 'ÖVRIGT');
    ...
    return {
      ...
      unit: packUnit,
      size: packSize,
      measureText: (typeof getMeasureTextFromSize === 'function') ? getMeasureTextFromSize(packSize, packUnit) : `${packSize} ${packUnit}`,
      weightText: (typeof isWeightUnit === 'function' && isWeightUnit(packUnit) && typeof getMeasureTextFromSize === 'function') ? getMeasureTextFromSize(packSize, packUnit) : '',
      ...
      img: sourceItem.img ? String(sourceItem.img) : '',
      openedAmount: 0,
      packMeasureUnit: packUnit,
      packMode: 'bags'
    };
  }

  const originalAddHomeItemFromTemplateV66 = window.addHomeItemFromTemplate || addHomeItemFromTemplate;
  window.addHomeItemFromTemplate = function addHomeItemFromTemplateV66(sourceItem, quantity = 1, targetPlace = null) {
    if (sourceItem && typeof isPackUnit === 'function' && isPackUnit(sourceItem.unit || '')) {
      const converted = clonePackTemplateToHomeV66(sourceItem, quantity, targetPlace);
      if (!converted) return;

      const existing = Array.isArray(items)
        ? items.find(entry => entry && entry.type === 'home' && typeof sameVariant === 'function' && sameVariant(entry, converted))
        : null;

      if (existing) {
        existing.quantity = Math.max(0, Number(existing.quantity || 0)) + Math.max(1, Number(converted.quantity || 1));
        existing.price = Number(converted.price || existing.price || 0);
        existing.category = converted.category;
        existing.place = converted.place;
        existing.room = converted.room;
        existing.size = converted.size;
        existing.unit = converted.unit;
        existing.measureText = converted.measureText;
        existing.weightText = converted.weightText;
        existing.packMeasureUnit = converted.packMeasureUnit || existing.packMeasureUnit || converted.unit;
        existing.packMode = 'bags';
        if (converted.img && !existing.img) existing.img = converted.img;
        if (typeof syncQuickItemFromItem === 'function') syncQuickItemFromItem(existing);
      } else if (Array.isArray(items)) {
        items.push(converted);
        if (typeof syncQuickItemFromItem === 'function') syncQuickItemFromItem(converted);
      }
      return;
    }
    return originalAddHomeItemFromTemplateV66.apply(this, arguments);
  };
  addHomeItemFromTemplate = window.addHomeItemFromTemplate;

  const originalSyncQuickItemFromItemV66 = window.syncQuickItemFromItem || syncQuickItemFromItem;
  window.syncQuickItemFromItem = function syncQuickItemFromItemV66(changedItem) {
    const quick = Array.isArray(quickItems)
      ? quickItems.find(q => (typeof normalizeText === 'function' ? normalizeText(q.name) : String(q.name || '').toLowerCase()) === (typeof normalizeText === 'function' ? normalizeText(changedItem?.name) : String(changedItem?.name || '').toLowerCase()))
      : null;

    if (quick && typeof isPackUnit === 'function' && isPackUnit(quick.unit || '') && changedItem && changedItem.type === 'home') {
      const packUnit = (typeof getPackMeasureUnit === 'function') ? getPackMeasureUnit(quick, 'g') : 'g';
      const changedUnit = String(changedItem.unit || '').toLowerCase();
      const changedSize = Math.max(0, Number(changedItem.size || 0));
      quick.room = changedItem.room || quick.room || (typeof activeRoom !== 'undefined' ? activeRoom : 'koket');
      quick.place = (typeof ensurePlaceExists === 'function') ? ensurePlaceExists(changedItem.place || quick.place || 'kyl', quick.room) : (changedItem.place || quick.place || 'kyl');
      quick.category = (typeof ensureCategoryExists === 'function') ? ensureCategoryExists(changedItem.category || quick.category || (typeof getRoomFallbackCategory === 'function' ? getRoomFallbackCategory(quick.room) : 'ÖVRIGT'), quick.room) : (changedItem.category || quick.category || 'ÖVRIGT');
      if (!quick.img && changedItem.img) quick.img = changedItem.img;
      if (Number(changedItem.price || 0) > 0) quick.price = Number(changedItem.price || 0);
      quick.unit = 'pkt';
      quick.packMeasureUnit = packUnit;
      if ((changedUnit === packUnit || (typeof isWeightUnit === 'function' && isWeightUnit(changedUnit) && isWeightUnit(packUnit)) || (typeof isLiquidUnit === 'function' && isLiquidUnit(changedUnit) && isLiquidUnit(packUnit))) && changedSize > 0) {
        quick.size = changedSize;
      }
    ...
    return originalSyncQuickItemFromItemV66.apply(this, arguments);
  };
  syncQuickItemFromItem = window.syncQuickItemFromItem;

  const originalUseQuickItemV66 = window.useQuickItem || useQuickItem;
  window.useQuickItem = function useQuickItemV66(index) {
    const item = Array.isArray(quickItems) ? quickItems[index] : null;
    if (!item) return;
    if (typeof isPackUnit === 'function' && isPackUnit(item.unit || '')) {
      window.addHomeItemFromTemplate(item, 1, item.place || 'kyl');
      try { if (typeof save === 'function') save(); } catch (error) {}
      try { if (typeof render === 'function') render(); } catch (error) {}
      return;
    }
    return originalUseQuickItemV66.apply(this, arguments);
  };
  useQuickItem = window.useQuickItem;

  window.__autoBuyZip = 'v66-pack-home-sync';
})();
...
(function () {
  ...
  function findQuickPackTemplateV67(source) {
    const name = typeof source === 'string' ? source : source?.name;
    if (!name || !Array.isArray(quickItems)) return null;
    return quickItems.find(item => item && nrm(item.name) === nrm(name) && typeof isPackUnit === 'function' && isPackUnit(item.unit || '')) || null;
  }

  function getPackSizeV67(template) {
    if (!template) return 0;
    const packUnit = (typeof getPackMeasureUnit === 'function') ? getPackMeasureUnit(template, 'g') : 'g';
    const parsed = (typeof parseSmartMeasureInput === 'function')
      ? parseSmartMeasureInput(template.measureText || template.weightText || template.size, packUnit)
      : null;
    return Math.max(1, Number(parsed || template.size || 0));
  }

  function makeBuyPackFromTemplateV67(template) {
    if (!template) return null;
    const roomKey = template.room || (typeof activeRoom !== 'undefined' ? activeRoom : 'koket');
    const packUnit = (typeof getPackMeasureUnit === 'function') ? getPackMeasureUnit(template, 'g') : 'g';
    const size = getPackSizeV67(template);
    return {
      name: template.name || '',
      price: Number(template.price || 0),
      quantity: 1,
      unit: 'pkt',
      size,
      measureText: (typeof getMeasureTextFromSize === 'function') ? getMeasureTextFromSize(size, packUnit) : `${size} ${packUnit}`,
      weightText: (typeof isWeightUnit === 'function' && isWeightUnit(packUnit) && typeof getMeasureTextFromSize === 'function') ? getMeasureTextFromSize(size, packUnit) : '',
      category: (typeof ensureCategoryExists === 'function') ? ensureCategoryExists(template.category || (typeof getRoomFallbackCategory === 'function' ? getRoomFallbackCategory(roomKey) : 'ÖVRIGT'), roomKey) : (template.category || 'ÖVRIGT'),
      place: (typeof ensurePlaceExists === 'function') ? ensurePlaceExists(template.place || 'kyl', roomKey) : (template.place || 'kyl'),
      room: roomKey,
      type: 'buy',
      img: template.img ? String(template.img) : '',
      packMeasureUnit: packUnit,
  ...
  function addOnePackToBuyV67(template) {
    const buyItem = makeBuyPackFromTemplateV67(template);
    if (!buyItem) return false;

    const existing = Array.isArray(items)
      ? items.find(entry => entry && entry.type === 'buy' && nrm(entry.name) === nrm(buyItem.name) && ((typeof isPackUnit === 'function' && isPackUnit(entry.unit || '')) || entry.packMode === 'bags'))
      : null;

    if (existing) {
      existing.quantity = Math.max(0, Number(existing.quantity || 0)) + 1;
      existing.unit = 'pkt';
      existing.size = buyItem.size;
      existing.measureText = buyItem.measureText;
      existing.weightText = buyItem.weightText;
      existing.packMeasureUnit = buyItem.packMeasureUnit;
      existing.price = Number(buyItem.price || existing.price || 0);
      existing.category = buyItem.category;
      existing.place = buyItem.place;
      existing.room = buyItem.room;
  ...
  function removeOnePackFromListV67(type, template) {
    if (!Array.isArray(items) || !template) return false;
    const normalizedType = type === 'buy' ? 'buy' : 'home';
    const nameKey = nrm(template.name);
    const index = items.findIndex(entry => entry && entry.type === normalizedType && nrm(entry.name) === nameKey && Number(entry.quantity || 0) > 0);
    if (index === -1) return false;

    const item = items[index];
    item.quantity = Math.max(0, Number(item.quantity || 0) - 1);

    if (normalizedType === 'home' && typeof isWeightUnit === 'function' && isWeightUnit(item.unit || '')) {
      item.packMode = (item.quantity > 1 || Number(item.openedAmount || 0) > 0 || item.packMode === 'bags') ? 'bags' : '';
      if (Number(item.quantity || 0) <= 0 && Number(item.openedAmount || 0) <= 0) {
        items.splice(index, 1);
      }
      return true;
    }
  ...
  function addOnePackToHomeV67(template, targetPlace) {
    if (!template) return false;
    if (typeof window.addHomeItemFromTemplate === 'function') {
      window.addHomeItemFromTemplate(template, 1, targetPlace || template.place || 'kyl');
      return true;
    }
    return false;
  }

  function persistAndRefreshV67() {
    try {
      if (typeof mergeItems === 'function') items = mergeItems(items);
    } catch (error) {}
    try {
      ...
      console.error('Persist error v67:', error);
    }
    try { if (typeof window.saveToCloudNow === 'function') window.saveToCloudNow(); } catch (error) {}
    try { if (typeof checkRecipe === 'function') checkRecipe(); } catch (error) {}
    try { if (typeof render === 'function') render(); } catch (error) {}
  ...
  const prevTransferSingleItemV67 = window.transferSingleItem || transferSingleItem;
  window.transferSingleItem = function transferSingleItemV67(index, targetType, targetPlace = null) {
    const sourceItem = Array.isArray(items) ? items[index] : null;
    if (!sourceItem) return;
    const template = findQuickPackTemplateV67(sourceItem);
    const normalizedTargetType = targetType === 'buy' ? 'buy' : 'home';
    const sourceType = sourceItem.type === 'buy' ? 'buy' : 'home';

    if (template) {
      if (sourceType === 'home' && normalizedTargetType === 'buy') {
        if (removeOnePackFromListV67('home', template)) {
          addOnePackToBuyV67(template);
          persistAndRefreshV67();
        }
        return;
      }
      if (sourceType === 'buy' && normalizedTargetType === 'home') {
        if (removeOnePackFromListV67('buy', template)) {
          addOnePackToHomeV67(template, targetPlace || sourceItem.place || template.place || 'kyl');
          persistAndRefreshV67();
        }
        return;
      }
    ...
    const result = prevTransferSingleItemV67.apply(this, arguments);
    persistAndRefreshV67();
    return result;
  };
  transferSingleItem = window.transferSingleItem;

  const prevUseRecipeIngredientsV67 = window.useRecipeIngredients || useRecipeIngredients;
  window.useRecipeIngredients = function useRecipeIngredientsV67() {
    const recipe = typeof getSelectedRecipe === 'function' ? getSelectedRecipe() : null;
    if (!recipe) return;
    ...
    (Array.isArray(combinedEntries) ? combinedEntries : []).forEach((entry) => {
      const ingredient = typeof normalizeRecipeIngredient === 'function'
        ? normalizeRecipeIngredient(entry?.ingredient || entry)
        : (entry?.ingredient || entry);
      const template = findQuickPackTemplateV67(ingredient);
      if (template) {
        if (removeOnePackFromListV67('home', template)) {
          addOnePackToBuyV67(template);
          changedPack = true;
    ...
    if (changedPack || normalEntries.length) persistAndRefreshV67();
  };
  useRecipeIngredients = window.useRecipeIngredients;

  const prevAddMissingToBuyV67 = window.addMissingToBuy || addMissingToBuy;
  window.addMissingToBuy = function addMissingToBuyV67() {
    if (!currentRecipeMissing || !currentRecipeMissing.length) {
      if (typeof checkRecipe === 'function') checkRecipe();
    }
    if (!currentRecipeMissing || !currentRecipeMissing.length) return;
    ...
    currentRecipeMissing.forEach((entry) => {
      ...
      const template = findQuickPackTemplateV67(ingredient);
      if (template) {
        if (addOnePackToBuyV67(template)) changedPack = true;
      } else {
        normalMissing.push(entry);
      }
    });

    if (normalMissing.length && typeof addMissingEntriesToBuy === 'function') {
      addMissingEntriesToBuy(normalMissing);
    }

    if (changedPack || normalMissing.length) persistAndRefreshV67();
  };
  addMissingToBuy = window.addMissingToBuy;

  window.__autoBuyZip = 'v67-pack-actions-fix';
})();


/* === MAX UI PREMIUM PASS: mobile nav + native swipe + haptic feel === */
(function () {
  'use strict';

  const PAGE_META = {
    home: { icon: '🏠', label: 'Hemma' },
    buy: { icon: '🛒', label: 'Köp' },
    quick: { icon: '⚡', label: 'Snabb' },
    week: { icon: '📅', label: 'Vecka' },
    recipes: { icon: '🍽️', label: 'Recept' },
    manage: { icon: '⚙️', label: 'Mer' }
  };
  const SWIPE_MIN_PX = 48;
  const SWIPE_TRIGGER_RATIO = 0.16;
  const EDGE_RESISTANCE = 0.40;
  const PARALLAX_HEADER = 0.10;
  const PARALLAX_TABS = 0.18;
  ...
  function safePageName(page) {
    return typeof normalizePageName === 'function' ? normalizePageName(page) : (PAGE_NAMES.includes(page) ? page : 'home');
  }

  function getCurrentPageNamePremium() {
    const active = document.querySelector('[data-page].page-visible');
    return safePageName(active?.dataset?.page || localStorage.getItem('activeKitchenPage') || 'home');
  }

  function getPageNodes(name) {
    return qa(`[data-page="${safePageName(name)}"]`);
  }

  function getPageIndexPremium(name) {
    return PAGE_NAMES.indexOf(safePageName(name));
  }

  function getNeighborsPremium(name) {
    const i = getPageIndexPremium(name);
    return {
      prev: i > 0 ? PAGE_NAMES[i - 1] : null,
      next: i < PAGE_NAMES.length - 1 ? PAGE_NAMES[i + 1] : null
    };
  }

  function isInteractive(target) {
    return !!target.closest(
      'input, textarea, select, button, a, label, [contenteditable="true"], .modal, .modal-box, .week-meal-row, .suggestion-box'
    );
  }

  function setNodeStyles(nodes, transform, opacity, zIndex, transition = 'none') {
    nodes.forEach((node) => {
      node.classList.add('swipe-prepared', 'page-visible');
      node.style.display = '';
  ...
  function getParallaxTargets() {
    return qa('.app-header, .page-tabs');
  }

  function setParallax(dx, progress) {
    qa('.app-header').forEach((node) => {
      node.style.transform = `translate3d(${dx * PARALLAX_HEADER}px,0,0)`;
      node.style.opacity = String(1 - Math.min(0.12, progress * 0.12));
    });
    qa('.page-tabs').forEach((node) => {
      node.style.transform = `translate3d(${dx * PARALLAX_TABS}px,0,0)`;
      node.style.opacity = String(1 - Math.min(0.10, progress * 0.10));
    });
  }

  function resetParallax(animated = true) {
    getParallaxTargets().forEach((node) => {
      node.style.transition = animated ? 'transform 260ms cubic-bezier(.22,1,.36,1), opacity 260ms cubic-bezier(.22,1,.36,1)' : '';
  ...
  function ensurePremiumNav() {
    qa('[data-page-tab]').forEach((button) => {
      const page = safePageName(button.dataset.pageTab || 'home');
      const meta = PAGE_META[page] || { icon: '•', label: button.textContent.trim() };
      if (button.dataset.upgraded === '1') return;
      button.dataset.upgraded = '1';
      button.innerHTML = `
        <span class="page-tab-icon" aria-hidden="true">${meta.icon}</span>
        <span class="page-tab-label">${meta.label}</span>
      `;
      button.setAttribute('title', meta.label);
    });
  }

  function ensureSwipeDots() {
    if (document.getElementById('swipeDots')) return;
    ...
    PAGE_NAMES.forEach((page, index) => {
      ...
      dot.setAttribute('aria-label', `Gå till ${PAGE_META[page]?.label || ('sida ' + (index + 1))}`);
  ...
  function ensureEdgeShadows() {
    if (document.querySelector('.swipe-edge-shadow')) return;
    const left = document.createElement('div');
    left.className = 'swipe-edge-shadow swipe-edge-left';
    const right = document.createElement('div');
    right.className = 'swipe-edge-shadow swipe-edge-right';
    document.body.appendChild(left);
    document.body.appendChild(right);
  }

  function setEdgeShadow(dx) {
    const left = document.querySelector('.swipe-edge-left');
    const right = document.querySelector('.swipe-edge-right');
    const strength = Math.min(1, Math.abs(dx) / 140);
    if (left) left.style.opacity = dx > 0 ? String(strength) : '0';
    if (right) right.style.opacity = dx < 0 ? String(strength) : '0';
  }

  function clearEdgeShadows() {
    qa('.swipe-edge-shadow').forEach((node) => {
      node.style.opacity = '0';
    });
  }

  function updateSwipeUi() {
    const page = getCurrentPageNamePremium();
    document.body.dataset.activePage = page;
    qa('.swipe-dot').forEach((dot) => {
  ...
  function preparePages() {
    currentPageName = getCurrentPageNamePremium();
    const neighbors = getNeighborsPremium(currentPageName);
    currentNodes = getPageNodes(currentPageName);
    prevNodes = neighbors.prev ? getPageNodes(neighbors.prev) : [];
    nextNodes = neighbors.next ? getPageNodes(neighbors.next) : [];

    setNodeStyles(currentNodes, 'translate3d(0,0,0) scale(1)', '1', '4');
    if (prevNodes.length) setNodeStyles(prevNodes, 'translate3d(-22%,0,0) scale(.975)', '.52', '2');
    if (nextNodes.length) setNodeStyles(nextNodes, 'translate3d(22%,0,0) scale(.975)', '.52', '2');
  }

  function cleanupPages() {
    qa('[data-page]').forEach((node) => {
      node.classList.remove('swipe-prepared');
      node.style.transition = '';
      node.style.transform = '';
      node.style.opacity = '';
      node.style.zIndex = '';
    ...
    const active = getCurrentPageNamePremium();
    qa('[data-page]').forEach((node) => {
      node.classList.toggle('page-visible', node.dataset.page === active);
    });
    resetParallax(false);
    clearEdgeShadows();
    updateSwipeUi();
  ...
  function applyDrag(dx) {
    ...
    const hasPrev = prevNodes.length > 0;
    const hasNext = nextNodes.length > 0;
    let move = dx;

    if ((dx > 0 && !hasPrev) || (dx < 0 && !hasNext)) {
      move = move * (0.5 - Math.min(Math.abs(move) / 800, 0.3));
    } else {
      move *= 1;
    }

    const progress = Math.min(1, Math.abs(move) / width);
    const sideOffset = 22 - progress * 22;
    const sideScale = 0.975 + progress * 0.025;
    const sideOpacity = 0.52 + progress * 0.48;

    setNodeStyles(currentNodes, `translate3d(${move}px,0,0) scale(${1 - progress * 0.015})`, String(1 - progress * 0.14), '4');

    if (move > 0 && hasPrev) {
      setNodeStyles(prevNodes, `translate3d(${-sideOffset}%,0,0) scale(${sideScale})`, String(sideOpacity), '3');
    }
    if (move < 0 && hasNext) {
      setNodeStyles(nextNodes, `translate3d(${sideOffset}%,0,0) scale(${sideScale})`, String(sideOpacity), '3');
    }

    setParallax(move, progress);
    setEdgeShadow(move);
  }

  function animateBack() {
    const transition = 'transform 260ms cubic-bezier(.22,1,.36,1), opacity 260ms cubic-bezier(.22,1,.36,1)';
    setNodeStyles(currentNodes, 'translate3d(0,0,0) scale(1)', '1', '4', transition);
    if (prevNodes.length) setNodeStyles(prevNodes, 'translate3d(-22%,0,0) scale(.975)', '.52', '2', transition);
    if (nextNodes.length) setNodeStyles(nextNodes, 'translate3d(22%,0,0) scale(.975)', '.52', '2', transition);
    resetParallax(true);
    clearEdgeShadows();
    setTimeout(cleanupPages, 270);
  }

  function animateToPage(direction) {
    const targetPage = direction < 0 ? (getNeighborsPremium(currentPageName).next) : (getNeighborsPremium(currentPageName).prev);
    const targetNodes = direction < 0 ? nextNodes : prevNodes;
    if (!targetPage || !targetNodes.length) {
      animateBack();
      return;
    }

    const transition = 'transform 260ms cubic-bezier(.22,1,.36,1), opacity 260ms cubic-bezier(.22,1,.36,1)';
    setNodeStyles(currentNodes, `translate3d(${direction < 0 ? '-100%' : '100%'},0,0) scale(.985)`, '.70', '4', transition);
    setNodeStyles(targetNodes, 'translate3d(0,0,0) scale(1)', '1', '5', transition);
    setParallax(direction < 0 ? -120 : 120, 1);
    pulseHaptic();

    setTimeout(() => {
      if (typeof window.setActiveKitchenPage === 'function') {
        window.setActiveKitchenPage(targetPage);
      }
      cleanupPages();
    }, 230);
  }
  ...
  function onTouchEnd() {
    ...
    const dx = currentX - startX;
    const width = window.innerWidth || 1;
    const trigger = Math.abs(dx) >= SWIPE_MIN_PX && (Math.abs(dx) / width) >= SWIPE_TRIGGER_RATIO;

    if (trigger) animateToPage(dx < 0 ? -1 : 1);
    else animateBack();
  }
  ...
  document.addEventListener('DOMContentLoaded', initMaxUi);
  window.addEventListener('load', initMaxUi);
  document.addEventListener('touchstart', onTouchStart, { passive: true });
  document.addEventListener('touchmove', onTouchMove, { passive: true });
  document.addEventListener('touchend', onTouchEnd, { passive: true });
  document.addEventListener('touchcancel', onTouchEnd, { passive: true });

  const originalSetActiveKitchenPageMaxUi = window.setActiveKitchenPage;
  if (typeof originalSetActiveKitchenPageMaxUi === 'function') {
    window.setActiveKitchenPage = function patchedSetActiveKitchenPageMaxUi() {
      const result = originalSetActiveKitchenPageMaxUi.apply(this, arguments);
      updateSwipeUi();
      return result;
    };
  }

  window.addEventListener('hashchange', updateSwipeUi);
  window.addEventListener('resize', () => {
    if (!dragging) cleanupPages();
  });
})();
